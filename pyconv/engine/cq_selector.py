...
class CQSelector:
    ...
    def auto_cq(
        ...
        if family in ("av1_qsv", "hevc_qsv") and self.qsv_profile in self.qsv_profiles:
    ...
    def vmaf_target_search(
        ...
        ffmpeg_bin = self.ffmpeg_engine.ffmpeg_bin if self.ffmpeg_engine else "ffmpeg"
        ...
        if rc != 0 or not sample_ref.exists():
            logger.warning(f"[{job_id}] VMAF: nie można wyciąć sampla rc={rc}: {stderr[:200]}")
            return None
