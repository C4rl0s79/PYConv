...
try:
    ...
    from ..network.copyparty import CopypartyClient
...
def start_convert(app: "PlexConvertApp") -> None:
    ...
    # --- Shared callbacks (same bar for all GPUs) ---
    # Copy and upload bars are global (one bar each)
    def _copy_cb(val, label=""):
        set_pb(app, "copypb", val, "copyinfo", label)

    def _upload_cb(val, label=""):
        set_pb(app, "uploadpb", val, "uploadinfo", label)

    # Łącznie: shared counter across ALL GPU workers
    _done_shared = [0]
    _done_lock_shared = threading.Lock()

    def _file_done_cb(done_n, total_n):
        """Shared across both GPU workers — ignores per-worker done_n."""
        with _done_lock_shared:
            _done_shared[0] += 1
            n = _done_shared[0]
        pct = int(n / max(total, 1) * 100)
        set_pb(app, "totalpb", pct, "totalinfo", f"{n}/{total}")
    ...
    for enc, pb, pb_info, log_tag, _gpu_label, gpu_cq, dl_lock in configs:
        ...
        cfg = PipelineConfig(
            ...
            on_copy_progress=_copy_cb,
            on_encode_progress=make_encode_cb(),
            on_upload_progress=_upload_cb,
            on_file_done=_file_done_cb,
        )
